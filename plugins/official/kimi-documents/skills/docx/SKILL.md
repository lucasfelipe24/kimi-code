...
- Resolve plugin-relative paths from this file's location: the plugin root is two directories up (`../..`). Run DOCX route commands from this skill directory or use absolute paths.
- WIR engine is linux-x64 only and requires exact CPython 3.12; if unavailable, fall back to md2docx or Create. Create requires `dotnet`; if missing, fall back to md2docx.

# Part 1: Routing

The `Read` tool or plain-text extraction via Bash only extracts text from `.docx` — all formatting is lost. If the task involves appearance or structure, use WIR engine's `session.read()` instead.
