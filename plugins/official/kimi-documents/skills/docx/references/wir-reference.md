...
The engine is at `docx/scripts/engine/`. It is bundled for linux-x64 only and requires exact CPython 3.12 because the native extension is named `_core.cpython-312-x86_64-linux-gnu.so`. If import fails on another platform or Python version, use md2docx or the Create route instead.

Before importing, add `docx/scripts/` to Python path:
