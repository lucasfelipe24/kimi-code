...
This route requires `dotnet`. Run `../../scripts/setup.sh --json` first; if `dotnet` is unavailable, use md2docx instead. The `scripts/docx` entrypoint auto-detects the local dotnet environment.
...
Cover = floating background (generated from scratch, see §4.14) + title paragraphs + SectionProperties(TitlePage, zero margins).

The cover background is NOT a pre-existing file — you generate it yourself using Playwright + SVG/HTML before writing Program.cs. See §4.14 for the full design flow.
...
Backcover: same pattern. Cover/backcover text must visibly contrast with background and feel visually separate from body text — use significantly larger font, different color tier, and generous vertical spacing.
...
Cover/backcover backgrounds are generated via Playwright (HTML/SVG → 2× scale PNG → insert as floating anchor §4.5).

**You must create each background from scratch.** The scripts in `scripts/generate_*.py` are style references — read one to learn the HTML/SVG + Playwright screenshot technique, then write your OWN unique design with colors/shapes that match YOUR document's palette. Never call these scripts directly or reuse their exact output. Every document deserves a unique visual identity.

Design flow:
1. Decide the document's color scheme first
2. Read ONE `generate_*.py` script to learn the SVG technique (radial-gradient, curves, filters, etc.)
3. Write new HTML/SVG from scratch — different shapes, different colors, different composition
...
From Bash: `python3 your_script output_dir`. If Python dependencies are missing, install them only inside a virtual environment before running the script.

Style references (for inspiration only, do NOT call directly or copy verbatim):
