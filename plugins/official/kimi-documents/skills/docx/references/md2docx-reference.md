...
Optional:
  ...
  --output-dir DIR     Output directory (workspace path, specify explicitly)
```
...
Use citation data from the current working directory by default:

`./citation.jsonl`

If a citation JSONL is supplied with the Markdown, pass that exact file. Otherwise, use `--citation ./citation.jsonl` when it exists. If no citation file exists, proceed without mapped citations, preserve the visible citation markers, and tell the user that mapped citation data was unavailable.
...
Agent-produced MD may contain absolute paths that don't exist in the current workspace. Check and fix before conversion:

```bash
grep -oE '!\[.*?\]\(.*?\)' document.md | head -10
# Replace absolute path prefix with relative — adjust prefix based on your workspace
python3 - <<'PY'
from pathlib import Path
p = Path('document.md')
text = p.read_text(encoding='utf-8')
p.write_text(text.replace('/path/to/remove/', ''), encoding='utf-8')
PY
```
...
Run the plugin-level environment check first from the DOCX skill directory:

```bash
../../scripts/setup.sh --json
```

Pandoc must be available on the host for this route. When the host has no `pandoc`, install `pypandoc_binary` inside the venv and put its bundled binary on `PATH` (no system-wide install needed):

```bash
.venv-docs/bin/pip install pypandoc_binary
export PATH="$(pwd)/.venv-docs/lib/python"*/site-packages/pypandoc/files:$PATH"
```

Install Python packages only inside a virtual environment, for example:

```bash
python3 -m venv .venv-docs
.venv-docs/bin/pip install python-docx lxml
.venv-docs/bin/python scripts/md2docx/md2docx_convert.py <markdown_file> \
    --citation ./citation.jsonl \
    --style footnote \
    --output-dir <output-dir>
```
