...
- Install Python dependencies only inside a virtual environment, for example `python3 -m venv .venv-docs && .venv-docs/bin/pip install ...`.
- Resolve plugin-relative paths from this file's location: the plugin root is two directories up (`../..`).
- Run commands with the Bash tool. The HTML route needs Node.js plus Chromium/Playwright; the LaTeX route needs Tectonic or another LaTeX engine; the Process route needs Python plus PDF libraries in a venv.
...
**Check the plugin environment first, then use the route scripts:**

```bash
# From the pdf skill directory
../../scripts/setup.sh --json

# Convert HTML to PDF
node ./scripts/html_to_pdf.js input.html --output output.pdf

# Compile LaTeX to PDF
python3 ./scripts/compile_latex.py input.tex --output output.pdf

# Process an existing PDF with Python dependencies from a venv
python3 -m venv .venv-docs
.venv-docs/bin/pip install -U pikepdf pdfplumber pypdf
.venv-docs/bin/python ./scripts/pdf.py extract text input.pdf
```

The local `./scripts/pdf.sh check` may also be used for PDF-skill diagnostics, but start with the plugin-level `../../scripts/setup.sh --json`. Do not use the local auto-repair command in kimi-code because it performs global installs.

**Dependencies by route:**
- **HTML route**: Node.js, Playwright, Chromium
- **Process route**: Python 3 plus `pikepdf`, `pdfplumber`, and `pypdf` installed in a virtual environment
- **LaTeX route**: Tectonic or another LaTeX engine available on the system
