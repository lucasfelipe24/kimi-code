...
## Step 0: Check Environment and Prepare a Venv (Do First)
**Run immediately before using pdf.py commands** to verify Python availability.

```bash
../../scripts/setup.sh --json
```

The plugin-level setup script only checks status; it never installs anything. Install Python dependencies only inside a virtual environment:
...
Use `.venv-docs/bin/python` for PDF processing commands when the venv is needed.
...
```
.venv-docs/bin/python ./scripts/pdf.py <command> <subcommand> [options]
```
...
```bash
.venv-docs/bin/python ./scripts/pdf.py form info input.pdf
```
...
**Step 2: Fill form**
```bash
.venv-docs/bin/python ./scripts/pdf.py form fill input.pdf -o output.pdf -d '{"name": "John", "agree": "true", "country": "US"}'
```
...
**Extract text**:
```bash
.venv-docs/bin/python ./scripts/pdf.py extract text document.pdf
.venv-docs/bin/python ./scripts/pdf.py extract text document.pdf -p 1-3    # Pages 1-3 only
.venv-docs/bin/python ./scripts/pdf.py extract text document.pdf -p 1,3,5  # Specific pages
```

**Extract tables**:
```bash
.venv-docs/bin/python ./scripts/pdf.py extract table document.pdf
```
...
**Merge PDFs**:
```bash
.venv-docs/bin/python ./scripts/pdf.py pages merge a.pdf b.pdf c.pdf -o merged.pdf
```

**Split PDF**:
```bash
.venv-docs/bin/python ./scripts/pdf.py pages split document.pdf -o ./output_dir/
```

**Rotate pages**:
```bash
.venv-docs/bin/python ./scripts/pdf.py pages rotate document.pdf 90 -o rotated.pdf
.venv-docs/bin/python ./scripts/pdf.py pages rotate document.pdf 180 -o rotated.pdf -p 1-3  # Specific pages
```

**Crop pages**:
```bash
.venv-docs/bin/python ./scripts/pdf.py pages crop document.pdf 50,50,550,750 -o cropped.pdf
```
...
**Read metadata**:
```bash
.venv-docs/bin/python ./scripts/pdf.py meta get document.pdf
```

**Set metadata**:
```bash
.venv-docs/bin/python ./scripts/pdf.py meta set document.pdf -o output.pdf -d '{"Title": "My Document", "Author": "John Doe"}'
```
...
```bash
.venv-docs/bin/python ./scripts/pdf.py convert input.docx -o output.pdf
```
