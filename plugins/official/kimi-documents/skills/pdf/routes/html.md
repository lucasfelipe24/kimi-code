...
## Step 0: Check Environment (Do First)
**Run immediately before writing HTML** to verify Node.js and Chromium/Playwright availability.

```bash
../../scripts/setup.sh --json
```

The plugin-level setup script only checks status; it never installs anything. The HTML route requires Node.js plus Chromium/Playwright. If either is missing, report the missing dependency and follow the user's environment policy; do not assume global npm installs are available or appropriate.

Run this check with the Bash tool before writing HTML.
...
For Markdown-first research reports, source markers such as `[^123^]` map by exact numeric `id` to the citation JSONL supplied with the Markdown, or to `./citation.jsonl` in the current working directory when no file is supplied. If that file is missing, proceed without mapped citations, preserve unresolved markers, and warn the user that citation metadata was not found. Treat the Markdown as canonical content and the HTML as a presentation-only projection. Use the matched metadata to build whatever citation treatment fits the PDF. References are optional rather than mandatory for this handoff. If an ID cannot be resolved, preserve the marker and report it instead of inventing or dropping a source.
