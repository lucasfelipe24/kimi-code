...
Run the plugin-level environment check first:

```bash
../../scripts/setup.sh --json
```

The setup script only reports status; it never installs anything. The LaTeX route is optional and requires Tectonic or another LaTeX engine already available on the system. If no LaTeX engine is available, report that the LaTeX route is unavailable instead of installing tools into the user home directory.
