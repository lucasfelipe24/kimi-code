...
PPTD (PPT-DSL) is a YAML abstraction layer for PowerPoint presentations, used to describe, generate, and edit slides in an AI-friendly way. In kimi-code, the bundled CLI can convert existing PPTX files into PPTD, validate/repair PPTD directories, and render screenshot previews; exporting PPTD back to PPTX is not available in this environment.
