...
This section lists all commands supported by the bundled kimi-slides CLI. In kimi-code, resolve the CLI relative to the skill directory as `../../bin/kimi-slides` after running the plugin-level `../../scripts/setup.sh --json`; the binary is available only on linux-x64. This document is the authoritative reference! The guide returned by `kimi-slides --help` is incorrect — there is no need to run that command.

The CLI supports only these commands: `convert` (PPTX to PPTD), `check` (validate/repair a PPTD directory), and `screenshot` (render a PPTD directory to PNG/JPG previews). It does not export PPTD to PPTX.

## convert

```bash
../../bin/kimi-slides convert path/deck.pptx 
../../bin/kimi-slides convert path/deck.pptx -o path/output/ 
```
...
```bash
../../bin/kimi-slides check path/deck 
../../bin/kimi-slides check path/deck -p 1,3 -s all 
../../bin/kimi-slides check path/deck -p 2-10 --level keep 
../../bin/kimi-slides check path/deck --level auto 
```
...
```bash
../../bin/kimi-slides screenshot path/deck -o path/screenshots/
../../bin/kimi-slides screenshot path/deck -p 1,3,5 -o path/screenshots/
../../bin/kimi-slides screenshot path/deck -p 2-6 -o path/screenshots/
```
