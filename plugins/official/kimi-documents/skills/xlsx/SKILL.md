...
- Install Python dependencies only inside a virtual environment, for example `python3 -m venv .venv-docs && .venv-docs/bin/pip install openpyxl pandas`.
- Resolve plugin-relative paths from this file's location: the plugin root is two directories up (`../..`).
- Excel creation with openpyxl/pandas is available anywhere Python and the required venv packages are available. The native `./scripts/Xlsx` validation/PivotTable CLI is embedded in this skill and works only on linux-x64; `recheck` also requires LibreOffice (`soffice`). On other platforms, create workbooks normally but do not claim native validation was run.
...
- **Execution**: Use the Bash tool to run Python scripts with `python3`, installing dependencies only in a virtual environment

**✅ Validation & PivotTable Tools:**
- **Tool**: Xlsx (unified CLI tool for validation, recheck, pivot, etc.)
- **Execution**: Use the Bash tool for CLI commands

**🔧 Execution Environment:**
- Use the Bash tool with `python3` for Excel creation with openpyxl/pandas
- Use the Bash tool for validation commands
- Create or reuse a project-local virtual environment for Python dependencies, for example `python3 -m venv .venv-docs && .venv-docs/bin/pip install openpyxl pandas`
...
- **🚨 This applies to ALL external tools**: WebSearch, FetchURL, API calls, or any fetched data
- If `./citation.jsonl` exists in the current working directory, use it to map fetched sources into citations. If it does not exist, proceed without mapped citations, cite sources from the available URLs/names, and tell the user that no citation mapping file was available.
...
The Xlsx tool has **6 commands** that can be called using the Bash tool:

**Executable Path**: `./scripts/Xlsx` (embedded in this skill; native CLI available only on linux-x64)
...
- **Known issue — `AbsolutePathInRels` from openpyxl**: some openpyxl/Python combinations (observed with openpyxl 3.1.5 on Python 3.14) emit package-absolute targets like `Target="/xl/worksheets/sheet1.xml"` in `xl/_rels/workbook.xml.rels`, which this validator rejects as critical. If `validate` fails ONLY with `AbsolutePathInRels`, rewrite the affected `.rels` targets to relative form (e.g. `Target="worksheets/sheet1.xml"`) — unzip, edit, rezip — and re-run `validate`; do not regenerate the workbook for this specific issue.
...
- Data is fetched from external sources (WebSearch, FetchURL, API)
...
- 🔗 **External Data Sources**: When using WebSearch, FetchURL, APIs, or any external data fetching tool, you MUST include source citations in the final Excel. Use `./citation.jsonl` in the current working directory when present; if it is missing, proceed with source names/URLs from the fetched data and tell the user no citation mapping file was available. Add `Source Name` and `Source URL` columns, or create a dedicated "Sources" sheet. ⛔ FORBIDDEN: Delivering Excel with fetched data but missing source references.
