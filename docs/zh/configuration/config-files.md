...
视觉模型是为**视觉类任务**单独配置的伴生模型——通常是一个具备视觉能力的模型，用来在主编码模型是纯文本时仍然能执行图像 / 截图 / 视频检查。它是默认绑定而非强制：解析器默认绑定所配置的视觉模型，执行视觉类工作的工具可以暴露 `model` 参数，仅接受 `"visual"`（已配置的视觉模型）与 `"primary"`（调用方自己的模型）两个符号值——与 subagent 模型选择的形态一致。未设置时，视觉任务绑定调用方模型。该节定义的是绑定契约而非接线：目前还没有工具消费它，单独配置它并不会重新路由图像检查——消费该绑定的媒体工具接线属于后续工作。
...
当 Brave 为当前后端且 API 密钥有效时，`WebSearch` 由 Brave 提供，engine v2 还会提供一组专用的 Brave 工具。工具列表见 [Brave Search 工具](../reference/tools.md#brave-search-工具)。请在官方 [Brave Search API 控制台](https://api-dashboard.search.brave.com/) 中申请 API 密钥并查看各端点的限流；定价和配额由 Brave 决定，而非 Kimi Code。
