...
| `KIMI_CODE_EXPERIMENTAL_DYNAMIC_WORKFLOWS` | 启用实验性的 Dynamic Workflow：main agent 获得 `Workflow` 工具，并会对大型多阶段任务自动进入 workflow 模式；master `KIMI_CODE_EXPERIMENTAL_FLAG=1` 也会启用它。见 [Dynamic Workflows](../customization/workflows.md) | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
