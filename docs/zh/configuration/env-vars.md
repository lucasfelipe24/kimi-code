...
| `KIMI_CODE_EXPERIMENTAL_DYNAMIC_WORKFLOWS` | 启用实验性的 Dynamic Workflow —— 经用户批准、按阶段编排 subagent 的 JavaScript 脚本；见 [Dynamic Workflows](../customization/workflows.md) | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
