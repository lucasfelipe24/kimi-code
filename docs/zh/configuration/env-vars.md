...
| `KIMI_CODE_EXPERIMENTAL_PERSISTENT_MEMORY_AUTO_EXTRACT` | engine v2 自动提取的 explicit-env opt-in；只生成待处理 proposal，不会自动写入。master `KIMI_CODE_EXPERIMENTAL_FLAG` 不会启用它 | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
