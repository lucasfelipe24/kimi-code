...
| `KIMI_CODE_EXPERIMENTAL_PERSISTENT_MEMORY` | Enable persistent memory in engine v2, including the `Memory` tool and selective recall | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
| `KIMI_CODE_EXPERIMENTAL_PERSISTENT_MEMORY_AUTO_EXTRACT` | Explicit-env opt-in for engine v2 automatic memory extraction; requires the persistent-memory flag and creates pending proposals only, never automatic writes. The master `KIMI_CODE_EXPERIMENTAL_FLAG` does not enable it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
