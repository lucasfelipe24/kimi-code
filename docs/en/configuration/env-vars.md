...
| `KIMI_CODE_EXPERIMENTAL_DYNAMIC_WORKFLOWS` | Enable experimental dynamic workflows: the main agent gains the `Workflow` tool and enters workflow mode automatically for large multi-phase tasks; the master `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it. See [Dynamic Workflows](../customization/workflows.md) | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
