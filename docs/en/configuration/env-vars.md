...
| `KIMI_VISUAL_MODEL` | Alias of the visual model used for image / screenshot / video inspection tasks; takes higher priority than [`[visual_model] model`](./config-files.md#visual_model) in `config.toml`. While set, it never leaks into `config.toml`. Read by the default `agent-core-v2` engine only | Alias of a configured `[models]` entry |
| `KIMI_VISUAL_EFFORT` | Thinking effort applied when visual tasks bind the visual model; takes higher priority than `[visual_model] default_effort` | An effort value, e.g. `low` |
