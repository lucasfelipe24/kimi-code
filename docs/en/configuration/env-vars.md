...
| `KIMI_CODE_EXPERIMENTAL_PERSISTENT_MEMORY_AUTO_EXTRACT` | Explicit-env opt-in for engine v2 automatic memory extraction; creates pending proposals only, never automatic writes. The master `KIMI_CODE_EXPERIMENTAL_FLAG` does not enable it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
...
| `KIMI_BRAVE_BASE_URL` | API URL of the Brave Search backend; takes higher priority than `[services.brave] base_url` in `config.toml`. Persisted credentials and custom headers are not forwarded to an env-selected endpoint. Read by the default `agent-core-v2` engine only | Non-blank string; blank values are ignored |
| `KIMI_BRAVE_API_KEY` | API key of the Brave Search backend; takes higher priority than `[services.brave] api_key`. Requires `active_search_provider = "brave"` to serve `WebSearch`. Read by the default `agent-core-v2` engine only | Non-blank string; blank values are ignored |
| `KIMI_CODE_EXPERIMENTAL_BRAVE_SEARCH` | Toggle the Brave Search backend (the `brave-search` flag), which is on by default; set it to a falsy value to turn Brave off. Effective on the default `agent-core-v2` engine only | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
| `KIMI_CODE_EXPERIMENTAL_LANGSEARCH_WEB_SEARCH` | Enable the experimental LangSearch web search backend and its optional semantic reranker (the `langsearch-web-search` flag); the master `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
