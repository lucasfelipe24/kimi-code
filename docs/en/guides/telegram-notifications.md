...
In the current version the integration supports only a flat private chat: one bot, one chat, and no forum topics or rich Markdown. Notifications, reply-driven turns, button answers, `/btw`, and the `TelegramSend` tool all work in this mode.
...
When asked, paste the bot token from BotFather. The command starts a short one-time poll; as soon as the first private chat messages the bot, the CLI stores that chat pairing in your global config. The poll lasts about 60 seconds.
...
To check whether the integration is configured, run:
...
You can also set the required values through environment variables, which take priority over the config file and are never written back to disk:
...
Inside the interactive TUI, type `/notify` to show a status notice with the current Telegram pairing. `/notify setup` tells you to run `kimi notify setup` in a shell; there is no in-TUI setup or toggle.
...
- **`/btw <question>`** starts an isolated side question. The answer comes back to Telegram, and neither the question nor the answer goes into the main session transcript. Sending exactly `/btw` with no question returns a usage message.
- **`TelegramSend`** is an agent tool that pushes a workspace file to the chat, with an optional caption.
...
- `kimi notify status` and `/notify` display a masked token, not the real value.
...
- [Built-in Tools](../reference/tools.md) — Full list of tools the agent can call, including `TelegramSend`
