...
| `/notify` | — | Show the current Telegram pairing status. `/notify setup` tells you to run `kimi notify setup` in a shell. See [Telegram notifications](../guides/telegram-notifications.md) | Yes |
...
`/workflow` (alias `/workflows`) manages [dynamic workflows](../customization/workflows.md) — JavaScript scripts that orchestrate subagents. The commands are available in the **CLI** and in the **Web UI** chat composer. Runs proposed by the model require your approval in `manual` permission mode; in `yolo` and `auto` modes they are approved automatically.
...
| `/memory` | — | On engine v2, manage persistent memories: list, create (`N`), edit (`E`), view (`Enter`), and forget (`D`). Create and edit open the memory as a markdown document in `$EDITOR` | Yes |
