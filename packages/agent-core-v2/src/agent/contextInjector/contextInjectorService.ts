...
export class AgentContextInjectorService extends Service implements IAgentContextInjectorService {
  ...
  private async reconcileAroundStep(
    ...
    await this.inject(ctx.firstStepOfTurn || rearmed, ctx.signal);
    await next();
    if (this.takeCompactionRearm()) {
      await this.inject(true, ctx.signal);
    }
  }
  ...
  private async inject(isNewTurn: boolean, signal?: AbortSignal): Promise<void> {
    for (const entry of this.entries) {
      signal?.throwIfAborted();
      await this.injectEntry(entry, isNewTurn, signal);
    }
  }

  private async injectEntry(
    ...
    try {
      content = await entry.provider(this.providerContext(entry, isNewTurn, signal));
      signal?.throwIfAborted();
    } catch (error) {
