...
import type { IWorkspaceMemoryCatalog } from '#/workspace/persistentMemory/memoryCatalog';
...
interface ProgramGeneration {
  ...
  readonly memory: IWorkspaceMemoryCatalog;
...
export class Program {
  ...
  get memory(): IWorkspaceMemoryCatalog { return this.requireGeneration().memory; }
  get processRunner(): ISessionProcessRunner { return this.requireGeneration().processRunner; }
  ...
  createSessionController(): SessionLifecycleService {
    ...
    try {
      ...
      return this.dependencies.createSessionController({
        ...
        processRunner: generation.processRunner,
        onDispose: release,
      });
    } catch (error) {
      release();
  ...
  private createGeneration(): ProgramGeneration {
    ...
    try {
      ...
      const memory = own(new WorkspaceMemoryCatalogService(this.context, this.dependencies.memoryStore, trust));
      const processRunner = own(new WorkspaceProcessRunnerService(this.context, runtime.process!));
      return {
