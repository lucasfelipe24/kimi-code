...
describe('AgentContextInjectorService', () => {
  ...
  it('passes the step signal to providers', async () => {
    let seen: AbortSignal | undefined;
    injector(ix).register('signal_test', ({ signal }) => {
      seen = signal;
  ...
  it('does not append provider content after the step is aborted', async () => {
    let release!: () => void;
    injector(ix).register('abort_test', async ({ signal }) => {
    ...
    const pending = loop.hooks.onWillBeginStep.run({
      turnId: 0,
      step: 0,
