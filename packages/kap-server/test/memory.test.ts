 ...
 *   - project scope on an untrusted workspace → 40930 (trust required)
...
describe('server-v2 /api/v1 memories', () => {
  ...
  describe('with the persistent-memory flag on', () => {
    ...
    it('rejects project-scope creation on an untrusted workspace with 40930', async () => {
      const wid = await registerWorkspace(await makeWorkspaceDir());
      ...
      expect(res.body.code).toBe(40930);
    });
