...
- For a terminal task, the metadata also explains why it ended: `status: timed_out` when a task was aborted by its deadline, and `stop_reason` when the task was explicitly stopped or failed. `terminal_reason` is a categorical label for the same event — its value is `timed_out`, `stopped`, or `failed` — and is emitted alongside the matching status / `stop_reason` field. A task that finished on its own with a clean exit emits neither `stop_reason` nor `terminal_reason`.
